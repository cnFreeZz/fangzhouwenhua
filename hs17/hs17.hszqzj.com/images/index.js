...
function getCodeInfo(code) {
  $.ajax({
    ...
    .done(function (r) {
      $('.gName,.btnBg span,.btnBgb span,.gpName,.tomorrow .red,.dialogBg span,h2 span,.btnBg2 span,.cc').html(r.info[code].name); //名称
      $('.gCode').text('( ' + code + ' )'); //代码
      $('.dm').text(code); //代码
      ...
      if (r.data[code]['199112'] >= 0) {
        ...
        $('.zhishu img').attr('src', '/common/images/arrow.png')
      } else {
        $('.zhangdie,.zhangfu,.xianjia').css('color', '#03a20c');
        $('.zhishu img').attr('src', '/common/images/arrowdown.png')
      }
      //停牌
      if (!r.data[code]['10']) {
        $('.xianjia').text('停牌');
...
//        });
//
//        $('.ddr-l').click(function(){
//          $('.comfir,.tan_div').fadeOut();
//        });
//        $('.ddr-r').click(function(){
//          $('.comfir').fadeOut();
//        })
